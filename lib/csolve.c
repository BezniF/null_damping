...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  valid_vars = 0;
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "Fd");
  if (xm == NULL) {
    printf("could not find params.Fd.\n");
  } else {
    if (!((mxGetM(xm) == 3) && (mxGetN(xm) == 1))) {
      printf("Fd must be size (3,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter Fd must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter Fd must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter Fd must be a full matrix.\n");
      this_var_errors++;
    }
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "Td");
  if (xm == NULL) {
    printf("could not find params.Td.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("Td must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter Td must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter Td must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter Td must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.Td;
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("tau must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  if (prepare_for_c) {
    ...
    for (i = 0; i < 3; i++)
      printf("  params.Fd[%d] = %.6g;\n", i, params.Fd[i]);
    for (i = 0; i < 1; i++)
      printf("  params.Td[%d] = %.6g;\n", i, params.Td[i]);
    for (i = 0; i < 1; i++)
